...
def compute_state_entropy(obs, full_obs, k):
    batch_size = 32
    obs_flat = obs.reshape(obs.shape[0], -1) / 255.
    full_obs_flat = full_obs.reshape(full_obs.shape[0], -1) / 255.
    with torch.no_grad():
        dists = []
        for idx in range(len(full_obs) // batch_size + 1):
            ...
            dist = torch.norm(
                obs_flat[:, None, :] - full_obs_flat[None, start:end, :], dim=-1, p=2
            )
            dists.append(dist)
...
class RadSacAgent(object):
    ...
    def reset_critic(self):
        self.critic = Critic(
            self.obs_shape, self.action_shape, self.hidden_dim, self.encoder_type,
            self.encoder_feature_dim, self.num_layers, self.num_filters).to(self.device)
        self.critic_target = Critic(
            self.obs_shape, self.action_shape, self.hidden_dim, self.encoder_type,
            self.encoder_feature_dim, self.num_layers, self.num_filters).to(self.device)
    ...
    def update_cpc(self, obs_anchor, obs_pos, cpc_kwargs, L, step, print_flag=False):
    ...
    def update_critic_state_ent(
        ...
        state_entropy = compute_state_entropy(state_obs, full_state_obs, k=K)
        if print_flag:
